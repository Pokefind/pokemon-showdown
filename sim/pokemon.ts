...
export class Pokemon {
	...
	constructor(set: string | AnyObject, side: Side) {
		...
		// Pokefind: don't truncate names in pokefind battles (custom display names can exceed 20 chars)
		...
		for (const moveid of this.set.moves) {
			...
			// Pokefind: persist PP/MaxPP across battles via startingPPs/startingMaxPPs on the set
			const moveIndex = this.baseMoveSlots.length;
			this.baseMoveSlots.push({
				move: move.name,
				id: move.id,
				pp: (this.set.startingPPs && this.set.startingPPs[moveIndex] !== undefined) ? this.set.startingPPs[moveIndex] : basePP,
				maxpp: (this.set.startingMaxPPs && this.set.startingMaxPPs[moveIndex] !== undefined) ? this.set.startingMaxPPs[moveIndex] : basePP,
				target: move.target,
				disabled: false,
				disabledSource: '',
				used: false,
		...
		// Pokefind: persist status across battles via set.status (sleep gets a randomized timer)
		if (this.battle.pokefindBattle && this.set.status) {
			this.status = this.battle.dex.conditions.get(this.set.status).id;
			this.statusState = this.battle.initEffectState({ id: this.status, target: this });
			if (this.status === 'slp') {
		...
		// Pokefind: skip EV/IV clamping in pokefind battles (custom stat ranges)
		if (!this.battle.pokefindBattle) {
		...
		this.clearVolatile();
		this.hp = this.maxhp;
		// Pokefind: persist HP across battles via startingMaxHP/startingHP on the set
		if (this.set.startingMaxHP !== undefined) this.maxhp = this.set.startingMaxHP;
		if (this.set.startingHP !== undefined) this.hp = this.set.startingHP;
	}
