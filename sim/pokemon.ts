	...
	*/
	...
	copyVolatileFrom(pokemon: Pokemon, switchCause?: string | boolean) {
		...
		for (const i in pokemon.volatiles) {
			...
			if (this.battle.dex.conditions.getByID(i as ID).noCopy) {
				continue;
			}
			// shallow clones
	...
	formeChange(
		...
		if (isPermanent && (!source || !["disguise", "iceface"].includes(source.id))) {
			...
			if (source || !this.getAbility().flags['cantsuppress']) this.setAbility(species.abilities['0'], null, null, true);
